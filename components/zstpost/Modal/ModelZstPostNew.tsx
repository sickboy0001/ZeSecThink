...
const ModelZstPostNew = (props: propTypes) => {
  const { showModal, date, putZstPosts } = props;
  ...
  const [formData, setFormData] = useState<TypeZstPostWithTags | null>(null);
  ...
  async function handleSubmit(event: any) {
    ...
    if (formData != null) {
      ...
      const newZstPost = await createZstPost({
        params: {
          ZstPost: formData,
        },
      });
      showModal(false);
      if (newZstPost) {
        // putZstPosts は TypeZstPostWithTags を期待する
        const postToUpdateState: TypeZstPostWithTags = {
          ...newZstPost, // createZstPost から返された基本情報
          tagIds: [],
        };
        putZstPosts(postToUpdateState, "insert"); // ★ 変換後のデータを渡す
      }
    }
...
export default ModelZstPostNew;
